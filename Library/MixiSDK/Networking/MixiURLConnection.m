...
+ (NSData *)sendSynchronousRequest:(NSURLRequest *)request returningResponse:(NSURLResponse **)urlResponse error:(NSError **)error {
    ...
    if (CFReadStreamOpen(readStream)) {
        ...
        do {
            ...
            NSString *string;
            if (bytes+leftover == sizeof(buf)) {
                leftover = 0;
                bytes = sizeof(buf);
                do {
                    string = [[NSString alloc] initWithBytes:buf length:bytes-leftover encoding:NSUTF8StringEncoding];
                    leftover++;
                    if (bytes <= leftover) break;
                } while (!string);
                leftover -= 1;
                [responseText appendString:string];
                [string release];
                if (0 < leftover) {
                    memmove(buf, &buf[sizeof(buf) - leftover], leftover);
                }
            }
            else {
                string = [[NSString alloc] initWithBytes:buf length:bytes encoding:NSUTF8StringEncoding];
                if (string == nil) break;                
                [responseText appendString:string];
                [string release];
            }
